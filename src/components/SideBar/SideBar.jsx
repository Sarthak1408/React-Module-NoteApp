...
const SideBar = ({setNoteActive, noteGroups, setSelectedNote, selectedNote, isPhone, display, setDisplay,}) => {
        ...
        >
            ...
            <div className="sidebarNotesList">
                <div className="notesList">
                    {noteGroups && noteGroups.map((note, index) => {
                        const notes = note.name.split(" ");
                        const firstLetters = notes.map((word) => word.charAt(0));
                        return (<div
                            className={`noteElement flex flex-row  justify-start ${note.id === selectedNote.id ? "selectedNote" : ""}`}
                            key={index}
